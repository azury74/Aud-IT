...
def activitees():
    ...
    for i in range(len(histo_faille["CVE ID"].values)):
        doc = nlp(histo_faille["Description"].values[i])
        ...
        for p in list_config:
            if(p in phrase):
                result2.append({"Nom":histo_faille["CVE ID"].values[i],"Date":histo_faille["Date"].values[i],"Description":histo_faille["Description"].values[i][0:100]+" ...","lien":histo_faille["References"].values[i]})
        
        
    resultot.append(result1)
    resultot.append(result2[0:5])
